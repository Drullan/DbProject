		...
		addButton.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				try{
					...
					String ingridient = fields[INGRIDIENT_NAME].getText().toLowerCase();
					if(ingridient.equals("")){
						messageLabel.setText("Error, please enter an ingridient name");
						return;
					}
					else if(ingridients.contains(ingridient)){
						messageLabel.setText("Error, this ingridient is already listed");
						return;
					}
					else if(!db.ingridientExists(ingridient)){
						if(!confirmDialog(ingridient)){
...
//		return mainpanel;
	...
	public boolean confirmDialog(String ingridient){
		Object[] options = {"Yes", "No"};
		int i = JOptionPane.showOptionDialog(null, "This ingridient doesn't exists in the database add it?", "Add ingridient",JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE,
				null, options, options[0]);
		if(i==0){
			db.addIngridient(ingridient, 0, "", 0);
			return true;
		}
		return false;
	}
	
	public boolean confirmUpdateDialog(String cookieName){
		Object[] options = {"Yes", "No"};
		int i = JOptionPane.showOptionDialog(null, "This product already exists! Do you want to update it with current values?", "Update product",JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE,
				null, options, options[0]);
		if(i==0){
			db.updateReciepeItems(cookieName, ingridients, quantities);
			return true;
		}
		return false;
	}
	...
	class ActionHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			switch(action){
			case 0:	
				String cookieName = fields[PRODUCT_NAME].getText().toLowerCase();
				...
				if(db.cookieExists(cookieName)){
					if(confirmUpdateDialog(cookieName)){
						JOptionPane.showMessageDialog(null,"Succesfully updated " + cookieName + "!");
						fields[PRODUCT_NAME].setText("");
						mainpanel.removeAll();
						ingridients.clear();
						quantities.clear();
						content();
						...
						messageLabel.setText("Error, this product already exists please rename it");
					}
				}else{
					db.addCookie(cookieName);
					db.addReciepeItems(cookieName, ingridients, quantities);
					fields[PRODUCT_NAME].setText("");
					mainpanel.removeAll();
					ingridients.clear();
					quantities.clear();
					...
					mainpanel.revalidate();
					mainpanel.repaint();
					JOptionPane.showMessageDialog(null,"Succesfully added " + cookieName + " to the database!");
				}
